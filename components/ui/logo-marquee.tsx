...
const FACE =
  "inline-flex h-14 shrink-0 items-center gap-2 whitespace-nowrap rounded-xl px-6 text-sm font-semibold tracking-[-0.02em] text-ash";

const HIT =
  "outline-none transition-colors duration-150 hover:text-obsidian focus-visible:bg-accent focus-visible:text-obsidian focus-visible:shadow-[inset_0_0_0_1px_var(--primary)]";
...
export function LogoMarquee({
  ...
  return (
    <section
      aria-label={label}
      className={`relative isolate w-full min-w-0 max-w-full overflow-hidden rounded-card bg-mist ${className}`}
      {...bind}
    >
      <div
        ref={viewportRef}
        ...
        className="overflow-y-hidden py-3 outline-none focus-visible:bg-accent"
      >
        <div
          ref={trackRef}
      ...
      <div
        aria-hidden
        className="pointer-events-none absolute inset-y-0 left-0 w-16 bg-gradient-to-r from-card to-card/0"
      />
      <div
        aria-hidden
        className="pointer-events-none absolute inset-y-0 right-0 w-16 bg-gradient-to-l from-card to-card/0"
      />
    </section>
  );
}
